...
namespace normal_math {
    ...
    namespace Math
    {
        ...
        [[nodiscard]] constexpr auto add_two_vectors(CONTAINER&& vector_one, CONTAINER&& vector_two) noexcept
        {
            auto result = std::remove_reference_t<CONTAINER>{};
            for(std::size_t index = 0; index < vector_one.size(); index++)
            {
                result.emplace_back(vector_one.at(index) + vector_two.at(index));
            }
            return result;
        }
        ...
        template<typename CONTAINER> requires containers::is_container_t<std::remove_reference_t<CONTAINER>>
        auto multiply_two_matrices(CONTAINER&& matrix_one, CONTAINER&& matrix_two, std::size_t number_of_rows_vector_one, std::size_t number_of_rows_vector_two, std::size_t number_of_columns_vector_two) noexcept
        {
            auto result = std::remove_reference_t<CONTAINER>(number_of_rows_vector_one * number_of_columns_vector_two);
            ...
            for(std::size_t b = 0; b < number_of_columns_vector_two; b++)
            {
                for(std::size_t m = 0; m < number_of_rows_vector_one; m++)
                {
                    ...
                    for(std::size_t n = 0; n < number_of_rows_vector_two; n++)
                    {
                        ...
                        value += matrix_one[m * number_of_rows_vector_two + n] * matrix_two[n * number_of_columns_vector_two + b];
                    }
